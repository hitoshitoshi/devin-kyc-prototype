...
- Decisions go through the `commitDecision` server action, which reads the record's current status from the server-side store (`src/lib/data/record-store.ts`, in-memory on `globalThis` — restarting the dev server resets dispositions; a page reload does not) and commits before emitting. Server-emitted events (`STATUS_UPDATED` incl. the Pending → Under Review auto-transition, `PII_UNMASKED`) survive a reload; client-only events (`VIEWED_RECORD`, `CHECKLIST_UPDATED`, `NOTE_ADDED`, `ROLE_SWITCHED`, `LEDGER_EXPORTED`), checklist state and notes do not. Confirm buttons show `Authorizing…` and are disabled while in flight. (`PII_UNMASKED` and `STATUS_UPDATED` are emitted server-side and appear in the ledger once the action resolves — allow a short delay before asserting). A denied decision shows a red `role=alert` banner inside the modal instead of closing it.
- Never run `next build` / `next start` in the same checkout while `next dev` is running — both write `.next` and the dev server starts 500ing (`ENOENT routes-manifest.json`). Kill processes by PID (`pgrep -af next-server`, then `kill <pid>`); `pkill -f next` from the exec tool kills the calling shell.
- The sandbox identity picker is always on under `npm run dev`; on a production build (`next start`) it is hidden unless `KYC_SANDBOX_IDP=enabled` (page shows `data-testid="sign-in-disabled"`).
