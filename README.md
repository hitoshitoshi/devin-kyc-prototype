...
| --- | --- | --- |
| **Auth / Session** (`middleware.ts`, `lib/auth/session*.ts`) | `/internal/*` is gated by Edge middleware that verifies an HTTP-only, HMAC-signed session cookie (`KYC_SESSION_SECRET`). The cookie carries `sub`, the active `role` and the operator's `grants`. Server actions (`revealSsn`, `beginReview`, `commitDecision`, `switchRole`, sign-in/out) reject requests whose `Origin` does not match the serving host, re-verify the cookie and never trust a role sent by the browser; the header switcher only offers granted roles and the server re-issues the cookie on change. | The browser cannot self-assign Compliance Lead. The sandbox IdP (`/internal/sign-in`) is the seam where SSO group claims plug in; it is development-only unless a deployment opts in with `KYC_SANDBOX_IDP=enabled`. |
...
| `KYC_SANDBOX_IDP` | Set to `enabled` to expose the pick-an-identity sandbox sign-in on a production build. Off by default in production: without it (or SSO) no session can be minted and `/internal/*` stays locked. Always on in development. |
